...
- **禁止将行级禁用作为长期解决方案**：
  ...
  - 允许在短期内为“降噪”添加禁用标记，但必须在 Taskmaster 任务体系中登记对应的重构任务，并在完成后移除禁用，恢复规则生效。
- **优先采用结构性重构消解告警**：
  - 对于函数过长、圈复杂度过高、文件超出约定行数等问题，应优先通过拆分函数/类型、引入 Pipeline 模式、提取适配器或 Domain 服务等结构性手段解决，而不是调低阈值或放宽规则。
  - 在 Godot+C# 栈中，复杂业务逻辑应拆分到 `Game.Core` 域层或 `Engine/Adapters` 中，场景脚本只保留装配与信号路由，避免通过在脚本中关闭分析规则来“压制”复杂度。
- **行级禁用必须具备“回收路径”**：
