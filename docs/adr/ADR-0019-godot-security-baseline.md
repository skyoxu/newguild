...
- Status: Accepted
- Context: 原 ADR-0002 针对 旧桌面壳 的安全基线（CSP、contextIsolation 等）已不再适用。Godot 运行时与文件系统/外链策略、插件与执行模型与 旧桌面壳 存在根本差异，需建立新的防御性基线并与质量门禁脚本协同。
- Decision:
...
- References: ADR-0011-windows-only-platform-and-ci, ADR-0003-observability-release-health, docs/architecture/base/02-security-baseline-旧桌面壳-v2.md（后续将以 Godot 版本替换）
