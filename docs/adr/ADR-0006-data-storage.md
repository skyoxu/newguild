...
本模板的运行时代码从 旧项目（旧桌面壳 + 旧前端框架 + SQLite + Node 脚本）迁移到 newguild（Godot 4.5 + C#）。原有 ADR‑0006 主要描述了 Node 侧 SQLite+WAL 策略，与当前 Godot 变体的实现存在以下差异：

- **平台差异**：现为 Windows-only，运行在 Godot 4.5 .NET runtime 上，不再依赖 Node/旧桌面壳 环境。
