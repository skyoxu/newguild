...
- 具体事件/DTO 与端口定义一律以 `Scripts/Core/Contracts/**` 为 SSoT；08 仅登记功能影响与测试范围。
...
- 公会生命周期事件：
  - `core.guild.created`（GuildCreated，契约位置规划：Scripts/Core/Contracts/Guild/GuildCreated.cs）
  - `core.guild.member.joined`（GuildMemberJoined，契约位置：Scripts/Core/Contracts/Guild/GuildMemberJoined.cs）
  - `core.guild.member.left`（GuildMemberLeft，契约位置规划：Scripts/Core/Contracts/Guild/GuildMemberLeft.cs）
...
注：如本模块引入新的事件或契约，请先更新 `Scripts/Core/Contracts/**`，再在 Overlay/08 以“引用”方式登记变更，并补充/更新 Test‑Refs。
