...
| **旧端到端测试工具** | E2E 回归测试自动化 | Web UI 测试 (仅适用于 HTML5/Web 版本) | `--mcp 旧端到端测试工具` |
...
# [PASS] 覆盖率: 95% (gate: 90%)
# [PASS] 场景测试: 3/3 通过
# [PASS] 安全烟测: allow/deny/invalid 验证通过
...
| 旧端到端测试工具 | E2E 回归测试 | `--mcp 旧端到端测试工具` (仅 Web 版) |
...
5. **MCP 工具最小化**: 默认仅启用 serena, 按需添加 context7/旧端到端测试工具
