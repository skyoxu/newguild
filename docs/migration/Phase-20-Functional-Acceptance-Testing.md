...
> **核心目标**：逐功能对标验证 旧项目 与 newguild 的特性一致性，建立功能完整性与兼容性验收基线。
> **工作量**：5-7 人天
...
| 挑战 | 原因 | Godot 解决方案 |
|-----|-----|-----------:|
| 功能对标验证 | 旧游戏引擎/旧前端框架 迁移到 Godot 可能遗漏功能 | 逐功能对标清单 + 自动化测试用例 |
| UI 行为差异 | 旧前端框架 与 Godot Control 交互模式不同 | Godot Control 节点层级测试 + 信号验证 |
...
> 本节说明：当前仓库是一个可复制的 Godot+C# 模板，并不存在一个一一对应的 旧项目 原始游戏版本。以下功能验收蓝图更多用于“未来实际游戏项目”作为检查清单，本模板阶段只要求核心骨干（Core + Scenes + Tests + Smoke/Perf）完整且可复用。
...
- 未在模板阶段实现的部分：
  - 与 旧项目 的逐功能对标（具体游戏关卡、角色/敌人行为、成就系统等）需要在实际项目中基于本模板扩展；
...
| 功能模块 | 旧项目 实现 | newguild 对标 | 验收标准 |
|---------|-------------|---------------|---------|
| **主菜单** | 旧前端框架 菜单组件 + EventBus | Godot Control 场景 | 菜单导航功能一致 |
| **游戏场景** | 旧游戏引擎 Scene Tree | Godot Scene Tree | 场景初始化、对象生成、销毁一致 |
| **角色控制** | 旧游戏引擎 精灵 + 输入系统 | Godot CharacterBody2D | 移动、跳跃、攻击行为一致 |
| **敌人 AI** | 旧游戏引擎 AI 逻辑 | Godot State Machine + FSM | 敌人行为逻辑一致 |
| **物理系统** | 旧游戏引擎 Physics | Godot Physics2D | 碰撞、重力、速度计算一致 |
...
| **数据导入** | - | 迁移脚本 | 旧项目 数据可导入 newguild |

#### 维度 D: 性能基线对标（Performance Baseline）

| 性能指标 | 旧项目 基线 | newguild 目标 | 验收标准 |
|--------|-------------|--------------|---------|
...
┌──────────────────────────────────────────────────────┐
│         Phase 20 功能验收测试启动                      │
│  依赖：Phase 1-19 完整实现 + newguild 可运行构建      │
└────────────────────┬─────────────────────────────────┘
                     │
        ┌────────────▼────────────┐
        │  第一阶段：冒烟测试      │
...
**职责**：
- 验证 newguild 核心游戏机制与 vitagame 的一致性
- 对标菜单、场景、数据系统等关键功能
...
namespace Game.Tests.Acceptance
{
    /// <summary>
    /// Phase 20 功能对标测试
    /// 验证 newguild 核心功能与 vitegame 的完整性与行为一致性
    /// </summary>
...
| 指标 | vitegame 基线 | newguild 目标 | 实际值 | 状态 |
|------|-------------|--------------|-------|------|
| 启动时间 | <2.0s | <2.5s | ____ |  |
| 场景加载 | <1.0s | <1.2s | ____ |  |
| FPS | 60 | 60 | ____ |  |
...
- [ ] smoke-tests.旧端到端测试工具.ts（300+ 行）：[OK] 冒烟测试脚本
...
- [OK] `tests/e2e/smoke-tests.旧端到端测试工具.ts`（300+ 行）
