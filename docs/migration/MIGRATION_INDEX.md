...
| 层次 | 原技术栈 (旧项目) | 新技术栈 (newguild) | 迁移复杂度 |

|------|-------------------|-------------------|----------|

| 桌面容器 | 旧桌面壳 | Godot 4.5 |  |

| 游戏引擎 | 旧游戏引擎 3 | Godot 4.5 (Scene Tree) |  |

| UI框架 | 旧前端框架 19 | Godot Control |  |

| 样式 | Tailwind CSS v4 | Godot Theme/Skin |  |

| 开发语言 | TypeScript | C# (.NET 8) |  |

| 构建工具 | 旧构建工具 | Godot Export Templates |  |

| 单元测试 | 旧单元测试工具 | xUnit + FluentAssertions |  |

| 场景测试 | - | GdUnit4 (Godot Unit Test) |  |

| E2E测试 | 旧端到端测试工具 (旧桌面壳) | Godot Headless + 自建Runner |  |

| 覆盖率 | 旧单元测试工具 Coverage | coverlet |  |
...
| 错误追踪 | Sentry (旧桌面壳 SDK) | Sentry (Godot SDK) |  |
...
- 旧桌面壳 安全基线 → Godot 安全基线（外链/网络/文件系统白名单）

- 旧前端框架 组件 → Godot Control 节点（UI 架构完全不同）

- 旧端到端测试工具 E2E → Godot Headless 测试（测试框架完全替换）
...
- 旧单元测试工具 单元测试 → xUnit 单元测试（测试框架迁移）

- 旧构建工具 构建流程 → Godot Export 流程（构建工具替换）
...
- [Phase-7-UI-Migration.md](Phase-7-UI-Migration.md) — 旧前端框架 → Godot Control 迁移
...
4. **分支并行**：保留 旧项目 主分支，newguild 在独立分支开发
...
- [PROJECT_DOCUMENTATION_INDEX.md](../PROJECT_DOCUMENTATION_INDEX.md) — 旧项目 完整文档索引
