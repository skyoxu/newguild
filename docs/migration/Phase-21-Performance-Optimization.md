...
**旧桌面壳 + 旧游戏引擎 3 性能基线**：
- 启动时间：冷启动 <2.0s（包含 旧桌面壳 初始化 + Node.js runtime）
- 帧时间：60 FPS @ 1080p（旧游戏引擎 WebGL 渲染，Chrome 引擎优化）
- 内存占用：峰值 ~200MB（包含 V8 堆 + 旧渲染内核 渲染进程）
- CPU 占用：空闲 <25%（主线程 + 渲染线程 + Web Worker）
- 资源加载：首屏资源 <500KB，懒加载策略

**已知性能瓶颈**：
- 旧桌面壳 容器开销：~50-80MB 基础内存（旧渲染内核 + Node.js）
- 旧前端框架 虚拟 DOM：复杂 UI 重绘可能触发卡顿（>16.67ms）
- 旧游戏引擎 场景切换：首次场景加载 ~800ms（纹理解压 + WebGL 初始化）
- SQLite 查询：复杂查询（JOIN）可能超过 50ms 阈值

### 新版（newguild）性能机遇与挑战

**机遇**：
- Godot 4.5 原生性能：无 旧桌面壳 容器开销，轻量级运行时（.NET 8）
...
5. **竞争力**：达到或超越 旧项目 性能基线（±25% tolerance）
