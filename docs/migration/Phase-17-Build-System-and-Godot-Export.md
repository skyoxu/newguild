...
- `npm run build` → 预构建 旧前端框架/Tailwind/旧游戏引擎（~2.5s）

- `旧桌面壳-builder` 自动化打包（签名、代码注入、资源优化）

- 产出：旧项目-1.0.0.exe（~150MB，包含 Node.js runtime）
...
> 本节描述的是 **当前 newguild 模板已经落地的构建/导出能力**。下文所述的 `scripts/build_windows.py`、独立 Release 工作流等仍处于蓝图阶段，对应增强统一收敛到 Phase-17 Backlog。
...
export_path="dist/newguild-1.0.0.exe"
...
export_path="dist/newguild-1.0.0-debug.exe"
...
class GodotBuildDriver:
    ...
    def clean_build_dir(self) -> None:
        ...
        if self.dist_dir.exists():
            ...
            for file in self.dist_dir.glob("newguild-*"):
    ...
    def build_export(self) -> Tuple[bool, Optional[str]]:
        ...
        exe_name = f"newguild-1.0.0{'-debug' if self.build_config == 'debug' else ''}.exe"
    ...
    steps:
      ...
      - name: Build Windows Executable (Release)
        ...
        run: |
          ...
          Get-Item dist/newguild-*.exe | ForEach-Object {
        ...
        with:
          ...
          body: |
            ...
            **Commit**: ${{ github.sha }}
            ...
            - `newguild-*.exe` - Windows Desktop executable

            - `newguild-*-SHA256.txt` - File integrity checksum
            ...
            Extract and run `newguild-1.0.0.exe`. No installation required.
...

{
...

    {
        ...
        public static string UserAgent =>
            ...
            $"newguild/{SemanticVersion} ({GitTag})";
...
# 输出：dist/newguild-1.0.0-debug.exe
...
# 输出：dist/newguild-1.0.0.exe, dist/build-metadata.json, dist/newguild-*-SHA256.txt
...
output = pathlib.Path('dist')/ 'newguild.exe'
...
用法：
```
py -3 scripts/sign_executable.py --file dist\newguild.exe --thumbprint <CERT_SHA1>
```
