...
本文件只关注 **与 Godot+C# 模板直接相关** 的能力；旧桌面壳/旧前端框架 专用能力会统一归入“未实现/未迁移（旧项目 专用）”。

## 0. 状态标记约定

- **Implemented**：模板中已经有对应 **代码/脚本/工作流**，并在 Phase 文档或 ADR 中有 Godot 变体说明。
- **Backlog**：能力已经在某个 Phase Backlog 文档中明确描述，但当前模板未实现（供后续项目按需落地）。
- **NotImplemented / NotMigrated**：只在旧的 旧项目/旧桌面壳 文档中出现，当前 Godot 模板既未实现，也未被纳入 Backlog（通常是 旧桌面壳/旧前端框架 专用能力）。
...
- 能力：技术栈与平台策略（ADR-0001/0011）  
  状态：**Implemented**  
  说明：已在 ADR 中明确 Windows-only + Godot+C# 技术栈；`ADR-0001-tech-stack.md` 仍保留部分 旧桌面壳/旧前端框架 描述，但在 `ADR-0011-windows-only-platform-and-ci.md` 与 Phase-17 文档中已经收敛为 Windows + Godot 架构。
...
- 能力：旧桌面壳 安全基线（ADR-0002 + CH02 原版口径）  
  状态：**NotImplemented / NotMigrated（旧项目 专用）**  
  说明：
  - 原文针对 旧桌面壳（CSP、contextIsolation 等），当前 Godot 模板安全基线已重写为 Godot 文件系统/网络/插件安全（见 Phase-14 文档与 Backlog）；
  - 旧桌面壳 相关条目保留为历史参考，但不适用于本模板。
...
- 未实现/未迁移（NotImplemented / NotMigrated，主要为 旧项目/旧桌面壳 专用能力）：
  - 旧桌面壳 主进程/渲染进程安全策略与前端打包流程；
  - 旧前端框架/旧游戏引擎 前端组件与相关测试/构建工作流；
...
2. 对 旧桌面壳/旧项目 专用能力，若未来不计划迁移到 Godot，可在相应 Phase 文档中显式标注为“历史参考，不适用于 Godot 模板”，进一步减少歧义。
