...
可选（推荐）：将 Godot `user://` 重定向到仓库 `logs/`（避免 `%APPDATA%\Godot\app_userdata\...` 的巨型日志无限增长）。

> 注意：这里填写的是**文件系统路径**，不要填 `user://` / `res://`，否则在 Windows 上会导致 `...\user:` 这类非法路径并触发 IOException。

```powershell
# 例如：把 user:// 写入指向仓库内目录（可按项目/CI 分组）
setx GODOT_USERDIR "C:\buildgame\newguild\logs\_godot_userdir\Godot Tests\ci"

# 可选：强制指定 Godot CLI 的 userdir 参数名（不确定时可不设；脚本会尝试 --help 自动探测）
setx GODOT_USERDIR_FLAG "--user-data-dir"
```
...
> - 如需清理（高风险操作）：`py -3 scripts/python/godot_userlog_manager.py --project Tests.Godot --purge --confirm PURGE`
...
py -3 scripts/python/validate_audit_logs.py --log-path "logs/ci/$date/security-audit*.jsonl" --check-sensitive --strict --report "logs/ci/$date/audit-validation-report.json"
```
