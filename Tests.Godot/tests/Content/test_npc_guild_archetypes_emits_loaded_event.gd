...
const EVENT_TYPE: String = "core.content.npc_guild_archetypes.loaded"
...
# ACC:T41.1
func test_main_scene_emits_npc_guild_archetypes_loaded_event_on_startup() -> void:
	var main := preload("res://Game.Godot/Scenes/Main.tscn").instantiate()
	add_child(main)
	auto_free(main)
	await get_tree().process_frame
	assert_bool(_got).is_true()
	assert_str(_source).is_equal("NpcGuildArchetypesBootstrapper")
	assert_bool(typeof(_payload) == TYPE_DICTIONARY).is_true()
	var dict := _payload as Dictionary
	assert_str(str(dict.get("contentVersion", ""))).is_not_empty()
	assert_int(int(dict.get("archetypeCount", 0))).is_greater(0)
