...
Default shape:
- task ids loaded from .taskmaster/tasks/tasks.json (numeric master task ids)
- batch size 5
- rounds 3
...
def default_out_path() -> Path:
    return resolve_repo_path(
        f"logs/ci/{today_str()}/sc-llm-obligations-jitter-batch5x3-raw.json"
    )
...
def read_task_outputs(
    ...
    *,
    ...
    if task_dir is None:
        summary = {"status": "fail", "rc": fallback_rc if fallback_rc != 0 else 1, "error": "missing_task_outputs"}
        verdict = {"status": "fail", "uncovered_obligation_ids": []}
        return None, summary, verdict

    summary_path = task_dir / "summary.json"
    verdict_path = task_dir / "verdict.json"
    summary_obj = read_json_safe(summary_path)
    verdict_obj = read_json_safe(verdict_path)
    if not isinstance(summary_obj, dict) or not isinstance(verdict_obj, dict):
        summary = {"status": "fail", "rc": fallback_rc if fallback_rc != 0 else 1, "error": "missing_or_invalid_outputs"}
        verdict = {"status": "fail", "uncovered_obligation_ids": []}
        return task_dir, summary, verdict
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--task-ids",
        default="",
        help="CSV task ids (e.g. 1,2,3 or T1,T2). Default: auto load numeric ids from --tasks-file.",
    )
    parser.add_argument(
        "--tasks-file",
