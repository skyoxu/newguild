...
def _write_logs(log_dir: Path, result: PatchResult) -> None:
    ...
    summary = {
        ...
        "overlay_dirs": result.overlay_dirs,
...
def main() -> int:
    ...
    result = PatchResult(
        ...
        overlay_dirs=sorted(inferred_dirs),
    )
    ...
    for overlay_dir in result.overlay_dirs:
        missing = _validate_overlay_files_exist(root, overlay_dir)
        if missing:
            missing_by_dir[overlay_dir] = missing

    if missing_by_dir:
        (log_dir / "missing-overlay-files.json").write_text(
            json.dumps({"missing_by_dir": missing_by_dir}, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        print("ERROR: required overlay anchor files missing for one or more overlay_dirs")
        for overlay_dir, missing in missing_by_dir.items():
            print(f"- {overlay_dir}: {missing}")
        return 2
