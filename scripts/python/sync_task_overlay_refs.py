...
def sync_view(view_path: Path, paths: OverlayPaths) -> tuple[list[dict[str, Any]], FileSyncResult]:
    ...
    for task in tasks:
        ...
        if not isinstance(raw_tm_id, int):
            skipped_ids.append(task_id or "<unknown>")
            continue
        expected = _refs_for_task(raw_tm_id, paths)
        if view_path.name == "tasks_back.json":
            if paths.index not in expected:
                expected.append(paths.index)
            if paths.acceptance not in expected:
                expected.append(paths.acceptance)
    ...
    if skipped_ids:
        print(
            f"SYNC_TASK_OVERLAY_REFS warn file={view_path.name} "
            f"skipped_non_int_taskmaster_id={len(skipped_ids)} ids={skipped_ids}"
        )
