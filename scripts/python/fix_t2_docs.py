#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix T2 guide and section 10 of task-master-superclaude doc.

This script:
- Rewrites docs/workflows/GM-NG-T2-playable-guide.md with an updated
  minimal task list (NG-0001/NG-0020/NG-0012/NG-0021/GM-0101/GM-0103).
- Replaces the mojibake section 10 in
  docs/workflows/task-master-superclaude-integration.md with a concise
  Chinese note explaining that Node/Web tooling is optional on top of
  the Godot+C# + Python toolchain.
...
def update_t2_guide() -> None:
    path = Path("docs/workflows/GM-NG-T2-playable-guide.md")
    if not path.exists():
        print("[fix_t2_docs] T2 guide not found, skip")
        return

    content = """# 从 GM/NG 任务驱动首个 T2 场景实现
...
"""
    ...
    print("[fix_t2_docs] updated GM-NG-T2-playable-guide.md")


def fix_taskmaster_section10() -> None:
    path = Path("docs/workflows/task-master-superclaude-integration.md")
    if not path.exists():
        print("[fix_t2_docs] task-master-superclaude-integration.md not found, skip")
        return
    ...
    if idx == -1:
        print("[fix_t2_docs] section 10 marker not found, skip")
        return

    new_section = """## 10. Node / Web 生态说明（可选附加）
...
"""
    ...
    print("[fix_t2_docs] replaced section 10 in task-master-superclaude-integration.md")


def main() -> None:
    update_t2_guide()
    fix_taskmaster_section10()
