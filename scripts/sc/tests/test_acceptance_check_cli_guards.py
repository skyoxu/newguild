...
class AcceptanceCheckCliGuardTests(unittest.TestCase):
    ...
    def test_task1_require_headless_should_not_force_post_gate_without_gd_refs(self) -> None:
        proc = subprocess.run(
            [
                sys.executable,
                str(SCRIPT),
        ...
        self.assertEqual("unit", tests_all.get("test_type"))

        headless = next((x for x in plan if isinstance(x, dict) and x.get("name") == "headless-e2e-evidence"), {})
        self.assertFalse(bool(headless.get("enabled")))
        self.assertEqual("not_required", headless.get("reason"))
        ...
        self.assertEqual("not_required", post_gate.get("reason"))
